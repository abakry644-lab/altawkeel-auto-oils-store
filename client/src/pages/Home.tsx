    ...
    <section className="relative overflow-hidden border-b border-[#e9e3d9] bg-[#f3f0e9]">
      ...
      <div className="container relative flex min-h-[460px] items-center justify-center py-16 sm:py-24 lg:py-28">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-[#60716a]">واجهة سهلة لاختيار زيوت المحرك والفلاتر الأساسية، مع تفاصيل واضحة وسلة عملية لإتمام طلبك براحة.</p>
          <div className="mt-8 flex flex-wrap justify-center gap-3"><Link href="/shop" className="inline-flex items-center gap-2 rounded-xl bg-[#163d36] px-5 py-3.5 text-sm font-extrabold text-white shadow-[0_12px_24px_rgba(22,61,54,.18)] transition hover:bg-[#285c50] active:scale-[.97]">تسوّق المنتجات <ArrowLeft size={18} /></Link><Link href="/shop?category=زيوت%20المحرك" className="inline-flex items-center gap-2 rounded-xl border border-[#d6cbbc] bg-white/80 px-5 py-3.5 text-sm font-extrabold text-[#25483e] transition hover:bg-white">استكشف الزيوت</Link></div>
          <div className="mx-auto mt-10 grid max-w-xl grid-cols-3 gap-3 border-t border-[#ddd6cc] pt-6"><div><strong className="block text-xl font-extrabold text-[#173b33]">01</strong><span className="mt-1 block text-xs font-bold text-[#79857f]">اختيار واضح</span></div><div><strong className="block text-xl font-extrabold text-[#173b33]">02</strong><span className="mt-1 block text-xs font-bold text-[#79857f]">سلة عملية</span></div><div><strong className="block text-xl font-extrabold text-[#173b33]">03</strong><span className="mt-1 block text-xs font-bold text-[#79857f]">دفع آمن</span></div></div>
        </div>
      </div>
    </section>
