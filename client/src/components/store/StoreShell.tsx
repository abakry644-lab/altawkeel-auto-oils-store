...
export default function StoreShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className="min-h-screen overflow-x-hidden bg-[#fbfaf7] text-[#1c2421]"
      dir="rtl"
    >
        ...
        <div className="container flex h-20 items-center justify-between gap-5">
          ...
          >
            ...
            <span className="leading-tight">
              <span className="brand-wordmark block text-lg font-extrabold tracking-tight text-[#163d36]">
                التوكيل
              </span>
              <span className="block text-[10px] font-bold tracking-[.12em] text-[#957d5e]">
                زيوت وفلاتر السيارات
              </span>
            </span>
          </Link>
          ...
          <nav className="hidden items-center gap-7 text-sm font-bold text-[#56635d] lg:flex">
            {navItems.map(item => (
              <Link
                key={item.href}
                href={item.href}
                className="transition-colors hover:text-[#bf8332]"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#bf8332] px-1 text-[11px] text-white">
                  {itemCount}
                </span>
              )}
            </button>
            ...
            <div className="container grid gap-1">
              {navItems.map(item => (
                <Link
                  ...
                  className="rounded-xl px-4 py-3 text-sm font-bold text-[#31534a] hover:bg-[#f0eee8]"
                >
                  {item.label}
                </Link>
              ))}
      ...
      <footer className="mt-20 bg-[#163d36] text-[#e9e7df]">
        <div className="container grid gap-10 py-12 md:grid-cols-[1.25fr_.8fr_.8fr]">
          <div>
            ...
            <p className="max-w-sm text-sm leading-7 text-[#c8d3cf]">
              وجهتك العملية لاختيار زيوت المحرك والفلاتر الأساسية بثقة ووضوح.
            </p>
          </div>
          <div>
            <h3 className="mb-3 text-sm font-extrabold text-[#d9a553]">
              التصنيفات
            </h3>
            <div className="grid gap-2 text-sm text-[#c8d3cf]">
          ...
          <div>
            ...
            <p className="text-sm leading-7 text-[#c8d3cf]">
        ...
        <div className="border-t border-white/10 py-5 text-center text-xs text-[#99aaa4]">
          ...
          <p className="mt-1.5 text-[11px] font-medium text-[#c6a263]">
            صُمّم بواسطة{" "}
            <span dir="ltr" className="font-bold tracking-wide text-[#e2bd73]">
              Eng/Ahmed Bakry
            </span>
          </p>
        </div>
